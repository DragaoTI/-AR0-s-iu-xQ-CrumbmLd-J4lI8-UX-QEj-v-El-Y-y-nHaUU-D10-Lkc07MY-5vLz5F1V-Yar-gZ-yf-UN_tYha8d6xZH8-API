    ...
    async def dispatch(self, request: Request, call_next: CALL_NEXT_TYPE) -> Response:
        ...
        try:
            ...
            if supabase_service and supabase_service.client:
                ...
                if current_user_id and not isinstance(current_user_id, str):
                    log_entry["user_id"] = str(current_user_id)
                elif current_user_id is None:
                     log_entry["user_id"] = None

                current_admin_id = log_entry.get("admin_id")
                if current_admin_id and not isinstance(current_admin_id, str):
                    log_entry["admin_id"] = str(current_admin_id)
                elif current_admin_id is None:
                    log_entry["admin_id"] = None
                
                if log_entry.get("request_body") is None: log_entry["request_body"] = None 
                
                # LOG ADICIONAL DO PAYLOAD ANTES DE INSERIR
            ...
            if hasattr(log_e, 'message') and log_e.message: # Tentar obter mais detalhes do APIError
                 print(f"   Detalhe do APIError (se houver): {log_e.message}")
            if hasattr(log_e, 'hint') and log_e.hint:
                 print(f"   Dica do APIError (se houver): {log_e.hint}")
            if hasattr(log_e, 'details') and log_e.details:
                 print(f"   Detalhes adicionais do APIError (se houver): {log_e.details}")
            import traceback
            traceback.print_exc()
